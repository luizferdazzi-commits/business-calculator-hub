...
const headings:Record<string,string>={'pt-br':'Guias práticos','es-mx':'Guías prácticas','es-cl':'Guías prácticas','de-de':'Praktische Ratgeber','en-in':'Practical guides','en-gb':'Practical guides','ja-jp':'実践ガイド'};
export default function GrowthGuideLinks({locale}:{locale:string}){const items=guideMap[locale]||[];if(!items.length)return null;return <section className="content"><h2>{headings[locale]||'Practical guides'}</h2><p>{items.map((x,i)=><span key={x.href}>{i?' · ':''}<Link href={x.href}>{x.label}</Link></span>)}</p></section>}
