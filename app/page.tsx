...
export default function Home(){return <main className={s.page}>
  ...
  <div className={s.wrap}><header className={s.header}><Link href="/" className={s.brand}><span className={s.logoBox}>▦</span><span>BUSINESS<br/><span className={s.brandBlue}>CALCULATOR HUB</span></span></Link><nav className={s.nav}><Link href="#calculators">Calculators</Link><Link href="/guides">Guides</Link><Link href="/ai-tools">AI Tools <span className={s.new}>NEW</span></Link><Link href="/advertise">Advertise</Link><Link href="/advertise" className={s.adNav}>📣 ADVERTISE WITH US</Link></nav></header></div>
  ...
  <div className={s.brWrap}>
    <section className={s.brHero}><div className={s.brHeroCopy}><h1>Financial tools and guides for <span>smarter decisions.</span></h1><p>100% free, built for entrepreneurs, freelancers and small businesses worldwide.</p><div className={s.brSearch}><span>e.g. ROI, freelance rate, margin, tax...</span><a href="#calculators">⌕</a></div><div className={s.brBenefits}><div><b>⚡</b><span><strong>100% Free</strong><small>No sign-up</small></span></div><div><b>✓</b><span><strong>Reliable</strong><small>Clear methodology</small></span></div><div><b>◷</b><span><strong>Fast & easy</strong><small>Results in seconds</small></span></div></div></div><div className={s.brCalcVisual}><div className={s.brCalcCard}><h3>Profit Margin Calculator</h3><label>Revenue<div>$ 10,000</div></label><label>Costs<div>$ 6,500</div></label><span>Profit Margin</span><strong>35.00%</strong><em>Excellent</em><Link href="/roi-calculator">View details →</Link></div><div className={s.brChart}><span>↗</span><div className={s.brBars}><i/><i/><i/></div><div className={s.brPie}/></div></div></section>
