...
export async function GET(){
 ...
 const opportunities=seeds.map((s,i)=>{const exists=paths.has(s.path);const clusterSignal=top.reduce((n:any,x:any)=>n+(String(x.path).toLowerCase().includes(s.cluster.split('/')[0].toLowerCase().replace('profit margin','margin'))?Number(x.views||0):0),0);const score=Math.max(1,100-i*2)+(s.commercial==='high'?15:5)+(clusterSignal>0?10:0)-(exists?60:0);return {...s,status:exists?'existing_or_covered':'content_gap',priorityScore:score,signalViews:clusterSignal}}).sort((a,b)=>b.priorityScore-a.priorityScore);
 return NextResponse.json({engine:'BCH Organic Growth Engine',generatedAt:new Date().toISOString(),cost:'zero additional paid services',sitemapPages:paths.size,aiReferrals:ai,topContent:top,topMarkets:(audience as any).topMarkets||[],opportunities,contentGaps:opportunities.filter(x=>x.status==='content_gap'),feedback:{weekly:(audience as any).weekly||null,last3Days:(audience as any).last3Days||null,nextAction:'Prioritize missing high-score pages; never auto-publish sensitive tax/legal facts without official-source validation.'}},{headers:{'Cache-Control':'public, s-maxage=60, stale-while-revalidate=300','X-Robots-Tag':'noindex, nofollow, noarchive'}})
}
